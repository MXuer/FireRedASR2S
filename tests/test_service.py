...
class SemanticAsrServiceTest(unittest.TestCase):
    ...
    def test_web_demo_route_uses_existing_job_api_and_multi_file_upload(self):
        ...
        self.assertIn('apiUpload("/v1/jobs"', demo)
        ...
        self.assertIn("const uploadConfig = configSelect.value", demo)
        self.assertIn('form.append("config", uploadConfig)', demo)
        self.assertIn("configSelect.disabled = true", demo)
        self.assertIn("formatInputs.forEach((input) => { input.disabled = true; })", demo)
        self.assertIn('id="upload-progress"', demo)
        self.assertIn('id="upload-progress-bar"', demo)
        self.assertIn("new XMLHttpRequest()", demo)
        self.assertIn("xhr.upload.onprogress", demo)
        self.assertIn("formatBytes(speed)", demo)
