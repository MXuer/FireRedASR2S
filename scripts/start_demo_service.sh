...
TRANSLATION_BATCH_SIZE="${SEMANTIC_ASR_TRANSLATION_BATCH_SIZE:-64}"
TRANSLATION_MAX_CONCURRENCY="${SEMANTIC_ASR_TRANSLATION_MAX_CONCURRENCY:-24}"
TRANSLATION_TIMEOUT_S="${SEMANTIC_ASR_TRANSLATION_TIMEOUT_S:-300}"
...
WORKERS_PER_DEVICE="${SEMANTIC_ASR_DEMO_WORKERS_PER_DEVICE:-2}"
...
if command -v curl >/dev/null 2>&1; then
  FIRST_TRANSLATION_BASE_URL="${TRANSLATION_BASE_URL%%,*}"
  if curl -fsS --max-time 2 "${FIRST_TRANSLATION_BASE_URL}/health" >/dev/null 2>&1; then
...
export SEMANTIC_ASR_TRANSLATION_TIMEOUT_S="${TRANSLATION_TIMEOUT_S}"
