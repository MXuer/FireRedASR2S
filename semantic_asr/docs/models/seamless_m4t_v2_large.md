...
For real runs, configure `src_lang` with the model's expected language code.
...
For transcription, the adapter defaults `tgt_lang` to `src_lang` so output
stays in the source language. Set `tgt_lang` explicitly only when translation
is intended.
...
```bash
conda run -n fireredasr2s python semantic_asr/examples/test_seamless_m4t.py --skip_model_load 1
```

Real model smoke test:

```bash
CUDA_VISIBLE_DEVICES=4 conda run -n fireredasr2s python semantic_asr/examples/test_seamless_m4t.py \
