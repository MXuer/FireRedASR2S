...
| `ru_ru` | Existing Russian profile and non-Chinese multilingual regression. | FireRed VAD + Whisper + Qwen3-ForcedAligner + ASR text punctuation; FunASR/Whisper/Qwen/Dolphin overlap; MMS forced aligner; XLM-R punctuation. |
...
| `th_th` | No-space script; useful for punctuation and alignment boundary stress. | FunASR, Whisper, Qwen3-ASR, Dolphin, Qwen3 aligner, MMS aligner and XLM-R punctuation. |
| `hi_in` | Indic script path where Qwen3-ForcedAligner is not available, forcing MMS coverage. | FunASR, Whisper, Qwen3-ASR, Dolphin, MMS forced aligner and XLM-R punctuation. |
...
| --- | --- |
| `ar_sa` | Right-to-left Arabic script; good for MMS, FunASR, Qwen3-ASR, Dolphin and XLM-R punctuation. |
...
| `pt_br` | Latin-script non-English sample with MMS region code and FunASR/Qwen overlap. |
