...
- For every project task, add or update TODO items in `tasks/todo.md` before implementation or investigation, then update each item as it is completed.
...
- For MMS/CTC alignment failures, always analyze to the concrete segment before changing code: identify the wav segment start/end, ASR text, prepared tokens, uroman/dictionary target length, repeat count, frame count and exact feasibility failure. Prefer fixing the upstream VAD/ASR/text-prep cause before adding downstream fallback, because fallback timestamps avoid crashes but are less accurate.
- When restarting the HTTP demo, restart or verify both the API process and its worker processes. An API-only restart leaves jobs stuck in `queued` even though `/health` is green.
